...
const getRandomArticles = () => {
  ...
  // 將 JSON 裡的所有文章扁平化為一個陣列，並篩選出 category:brown 的資料
  for (const line in featuredData) {
    const filteredArticles = featuredData[line].filter((article) => {
      return article && article.category === "brown"; // 確保 article 存在並且有 category
    });
...
<template>
  ...
  <div class="metroline-detail brown">
    ...
    <div class="top-section-mobile">
      <h1 class="banner-group-title">中和新蘆線</h1>
      <swiper
