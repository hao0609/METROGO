...
import station_blue from "../../assets/images/MessionGeneral/station_blue.vue";
...
provide("pinStyle_blue", pinStyle_blue); // 透過 `provide` 提供 `pinStyle_blue`
...
const updateLocation = async () => {
  try {
    ...
    if (result.No_Station == false) {
      // 拿取目前用戶已打卡的所有站點
      const userDB_AllDoneStations = await checkUserDB_AllDoneStations(
        user_status.value.uid
      );

      // 判斷目前用戶在附近的捷運站有沒有在已打卡的所有站點中
      if (
        userDB_AllDoneStations.some((station) => station.捷運站名稱 == result.station)
      ) {
        // 如果有，隱藏 pin
        pinStyle_blue.value = pinjs("").pinStyle_blue.value;
      } else {
        // 如果沒有，顯示 pin 該在的位置
        pinStyle_blue.value = pinjs(result.station).pinStyle_blue.value;
      }
    } else {
      pinStyle_blue.value = pinjs("").pinStyle_blue.value;
    }
...
<template>
      ...
      <div class="closeStation_Info" :style="locationInfobox_style">
        <div class="closeStation_Info_box">
          <div class="location_info">
            ...
            <div class="location">
              <div class="lat">
                <span class="tittle">緯度</span>
                <span class="value">{{ latitude }}</span>
              </div>
              <div class="lng">
                <span class="tittle">經度</span>
                <span class="value">{{ longitude }}</span>
              </div>
            </div>
          </div>
          <div class="nearStation">
            距離最近的捷運站是: <span class="value">{{ closestStation }}</span>
          </div>
          <div class="neardiff">
            距離約 <span class="value">{{ dis }}</span> 公尺
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="hitarea">
    <button
      ...
      class="pin"
      :style="pinStyle_blue"
      @click="UserLocationSuccessful"
      @touchstart="UserLocationSuccessful"
    >
      <pin />
    </button>
  </div>
  <alert_positioning_successful
    ref="alert_web_M_userlocation"
    :nearby_station="station_result"
  />
